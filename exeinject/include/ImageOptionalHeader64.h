...
inline auto operator<<(std::ostream& out, const ImageOptionalHeader64& header) noexcept -> std::ostream& {
  out
    << "\t\tMagic: PE32+" << std::endl
    << "\t\tMajor Linker Version: " << static_cast<int>(header.major_linker_version) << std::endl
    << "\t\tMinor Linker Version: " << static_cast<int>(header.minor_linker_version) << std::endl
